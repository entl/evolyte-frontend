...
import { type Ref, ref, watch, defineProps, defineEmits, computed } from 'vue'
...
// Ensure modelValue is initialized properly
const defaultStart = new CalendarDate(2022, 1, 1)
const defaultEnd = defaultStart.add({ days: 7 })

// Reactive date range state
const value = ref<DateRange>(props.modelValue ?? { start: defaultStart, end: defaultEnd })

// Computed property to avoid undefined dates
const safeValue = computed(() => ({
  start: value.value.start ?? defaultStart,
  end: value.value.end ?? defaultEnd
}))

const placeholder = ref(safeValue.value.start) as Ref<DateValue>
const secondMonthPlaceholder = ref(safeValue.value.end) as Ref<DateValue>
...
// Watch for value updates and emit changes
  ...
  <Popover>
    <PopoverTrigger as-child>
      ...
      >
        ...
        <template v-if="safeValue.start">
          <template v-if="safeValue.end">
            {{
              formatter.custom(toDate(safeValue.start), {
                dateStyle: "medium",
              })
            }}
            -
            {{
              formatter.custom(toDate(safeValue.end), {
                dateStyle: "medium",
              })
            }}
          </template>
          ...
          <div class="flex flex-col gap-4">
            ...
            <RangeCalendarGrid>
              ...
              <RangeCalendarGridBody>
                <RangeCalendarGridRow v-for="(weekDates, index) in firstMonth.rows" :key="index" class="mt-2 w-full">

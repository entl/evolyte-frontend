...
const form = reactive<SolarPredictionForm>({
  ...
  kwh_price: null,
  loss: 0,
  dateRange: {start: new CalendarDate(today.getFullYear(), today.getMonth() + 1, today.getDate()), end: new CalendarDate(today.getFullYear(), today.getMonth() + 1, today.getDate() + 1)}
});

watchEffect(() => {
  if (coords.value.latitude && coords.value.longitude && coords.value.latitude !== Infinity && coords.value.longitude !== Infinity) {
...
const co2SavedForPeriod = () => {
  ...
  return formatDecimalPoints(solarForecast.value.reduce((acc, curr) => acc + curr.co2_saved, 0), 2);
};

const moneySavedForPeriod = () => {
  if (!solarForecast.value) return 0;

  return formatDecimalPoints(solarForecast.value.reduce((acc, curr) => acc + curr.money_saved, 0), 2);
};
...
<template>
  <div class="h-screen flex flex-col">
    ...
    <!-- Main Content -->
    <main class="flex flex-col lg:flex-row gap-4 p-6 h-screen overflow-auto">
      <!-- Left Section: Map & Analytics -->
      <div class="flex flex-col flex-1 space-y-4 h-full">
        <!-- Map Section -->
        <div class="rounded-lg shadow-lg h-64 md:h-1/2 min-h-[250px]">
          <MapView
              :lat="form.lat" :lon="form.lon" @update:location="(val) => {
        ...
        <div class="flex flex-col lg:flex-row gap-4 h-full lg:h-1/2">
          <!-- Graph Card -->
          <ForecastChart class="h-full flex-[3]" :forecast-data="solarForecast"/>
          <div class="h-full flex 2xl:hidden">
      ...
      <div class="flex flex-col h-full w-full lg:w-[30%] gap-4">
        <ParametersForm v-model:form="form" @submit-form="onSubmitForm" />
        <!-- Weather Section -->
        <WeatherWidget class="h-1/3" :weather-data="currentWeatherData"/>

        <div class="hidden 2xl:block 2xl:h-1/3">
          <AggregatedSolarData :data="aggregatedSolarData" :cols="4" />
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 h-full lg:h-1/4">
          <Card class="shadow-lg">
            ...
            <CardContent class="flex items-center p-4 text-green-500 dark:text-green-400">
              <p class="text-2xl font-semibold 2xl:text-4xl">{{ co2SavedForPeriod() }} KGs</p>
            </CardContent>
          </Card>
          ...
          <Card class="shadow-lg">
            ...
            <CardContent class="p-4 text-primary">
              <p class="text-2xl font-semibold 2xl:text-4xl">£{{ moneySavedForPeriod() }}</p>
            </CardContent>
          </Card>
        </div>
      </div>
